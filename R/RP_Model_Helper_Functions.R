...
data_prep <- function(formula,
  ...
  if (!is.null(panel_ids)) X_ID <- as.vector(data[,panel_ids]) else X_ID <- NULL
  
  X_fixed <- as.matrix(modelr::model_matrix(data, formula))
  if (!is.null(rpar_formula)) X_rand_crosssectional <- as.matrix(modelr::model_matrix(data, rpar_formula)) else X_rand_crosssectional <- NULL
  if (!is.null(panel_rpar_formula)) X_rand_panel <- as.matrix(modelr::model_matrix(data, panel_rpar_formula)) else X_rand_panel <- NULL
  if (!is.null(underrport_formula)) X_underreport <- as.matrix(modelr::model_matrix(data, underrport_formula)) else X_underreport <- NULL
  if (!is.null(parmam1_formula)) X_param1 <- as.matrix(modelr::model_matrix(data, parmam1_formula)) else X_param1 <- NULL
  if (!is.null(param2_formula)) X_param2 <- as.matrix(modelr::model_matrix(data, param2_formula)) else X_param2 <- NULL
  if (!is.null(underreport_rpar_formula)) X_underreport_rpar <- as.matrix(modelr::model_matrix(data, underreport_rpar_formula)) else X_underreport_rpar <- NULL
  if (!is.null(het_means_formula)) X_het_means <- as.matrix(modelr::model_matrix(data, het_means_formula)) else X_het_means <- NULL
  if (!is.null(het_variance_formula)) X_het_variance <- as.matrix(modelr::model_matrix(data, het_variance_formula)) else X_het_variance <- NULL
  if (!is.null(offsets)) X_offsets <- as.matrix(data[,offsets]) else X_offsets <- NULL
  if (!is.null(panel_ids)) X_ID <- as.vector(data[,panel_ids]) else X_ID <- NULL
...
get_Nparams <- function(prepped_data, family="NB2"){ # inputs are the output from `data_prep` and the family name
  ...
  N_Offsets <- ifelse(is.null(prepped_data$X_offsets), 0, ncol(prepped_data$X_offsets))
...
get_start_vals <- function(formula, 
  ...
  ParamCounts <- get_Nparams(prepped_data, family)
  
  N_fixed <- ParamCounts$N_fixed 
  N_rand_crosssectional <- ParamCounts$N_rand_crosssectional 
  N_rand_panel <- ParamCounts$N_rand_panel 
  N_underreport <- ParamCounts$N_underreport
  N_param1 <- ParamCounts$N_param1 
  N_param2 <- ParamCounts$N_param2 
  N_underreport_rpar <- ParamCounts$N_underreport_rpar 
  N_het_means <- ParamCounts$N_het_means 
  N_het_variance <- ParamCounts$N_het_variance
  N_family_params <- ParamCounts$family_params
  N_rand_total <- ParamCounts$N_rand_total
  # N_Offsets <- ParamCounts$N_Offsets
  ...
  nb_vars <- modelterms[!grepl("ntercept", modelterms)] # remove the intercept
  ...
  varnameorder <- c(colnames(X_fixed), colnames(X_rand_crosssectional))
...
coef_vals <- function(coeffs, 
  ...
  if (N_rand_crosssectional>0) beta_crss_mean <- coeffs[rand_crssc_indices] else beta_rand_crosssectional <- NULL
  if (N_rand_panel>0) beta_panel_mean <- coeffs[rand_panel_indices] else beta_rand_panel <- NULL
  if (N_underreport>0) beta_underreport_rpar <- coeffs[underrep_rpar_indices] else beta_underreport_rpar <- NULL
  if (correlated) beta_chol <- coeffs[chol_indices] else beta_chol <- NULL
  if (!correlated) {
    if (N_rand_crosssectional>0) beta_crss_std <- coeffs[rand_crssc_std_indices] else beta_rand_crosssectional <- NULL
    if (N_rand_panel>0) beta_panel_std <- coeffs[rand_panel_std_indices] else beta_rand_panel <- NULL
    if (N_underreport_rpar>0) beta_underreport_rpar_std <- coeffs[underrep_rpar_std_indices] else beta_underreport_rpar_std <- NULL
  }
