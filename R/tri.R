...
#'
#' The Triangle Distribution is defined by three points: a (minimum), b (maximum), and c (mode),
#' where the density is zero outside the interval [a, b], increases linearly from a to c, and
#' decreases linearly from c to b.
#'
...
#' @param sigma single value or vector indicating both the positive and negative max differences from the mean (if the difference is the same).
#' @param upper single value or vector for the upper limit of the distribution (must be used with `lower`).
#' @param lower single value or vector for the lower limit of the distribution (must be used with `upper`).
...
#'
#' The mode and standard deviation parameters define the distribution's location and scale,
#' respectively, while the lower and upper bounds explicitly set the minimum and maximum
